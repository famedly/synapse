...
class MediaStorage:
    ...
    @trace_with_opname("MediaStorage.stage_incoming_file")
    @contextlib.asynccontextmanager
    async def stage_incoming_file(
        self, file_info: FileInfo
    ) -> AsyncIterator[Tuple[BinaryIO, str]]:
        """Async Context manager used to get a file like object to write into, as
        described by file_info. Recommend for incoming remote media only
        ...
        Example:
            ...
            async with media_storage.stage_incoming_file(info) as (f, fname,):
                # .. write into f ...
        """
        # Incoming media, usually remote, we will not have a proper sha256 until the
        # object has completely been transferred. Just save the file based on the legacy
        # media id path, and move it later(for the moment, that means after the
        # streaming is complete and after writing the metadata to the database, but
        ...
        try:
            with start_active_span("staging to main media repo"):
                with open(fname, "wb") as f:
                    yield f, fname
    ...
    def resolve_path_to_media_file(self, file_info: FileInfo) -> str:
        ...
        if file_info.sha256 and not file_info.url_cache:
